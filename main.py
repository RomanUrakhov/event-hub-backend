...
from src.api.mappers import event_from_usecase_to_api_response
...
from src.usecases.account_cases import (
    login_account,
    has_event_access,
    has_system_access,
)
...
def twitch_auth():
    ...
    try:
        login_response = login_account(
            auth_code=code, auth_provider=auth_provider, user_account_repo=account_repo
        )
...
def create_event():
    ...
    create_event_response = event_cases.create_event(
        event_data=event_data, event_repo=event_repo
    )
...
def get_events():
    events = event_cases.list_events(event_repo=event_repo)
    event_responses = []
    for event in events:
        event_response = event_from_usecase_to_api_response(event)
        event_responses.append(event_response)
    response = ListEventsResponse(events=event_responses)
...
def get_event(event_id: str):
    try:
        event = event_cases.get_event(event_id, event_repo=event_repo)
        response = event_from_usecase_to_api_response(event)
...
@require_authentication
@require_access(
    event_id_field="event_id", event_action=SpecificEventAction.UPDATE_METADATA
)
def update_event(event_id: str):
    ...
    event_repo = FakeEventRepository(events=[])
    event_cases.update_event(event_data=event_data, event_repo=event_repo)
...
def upload_image():
    ...
    return jsonify({"id": file_id, "url": f"/api/images/{file_id}"}), 201
